...
class PlayerTest < ActiveSupport::TestCase
  ...
  def test_should_not_rehash_password
    ...
    assert_equal players(:quentin), Player.authenticate('quentin2', 'monkey')
  end

  def test_should_authenticate_player
    assert_equal players(:quentin), Player.authenticate('quentin', 'monkey')
  end
...
protected
  def create_player(options = {})
    record = Player.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.save
    record
  end
end
